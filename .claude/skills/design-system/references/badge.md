...
| `tone`      | `leaf \| coral \| aqua \| violet \| sky \| sun` | —        | Semantic accent → token border + soft fill. Pick by meaning, not by look.   |
...
Tones are `--surface-*`-adjacent tokens (leaf, coral, aqua, violet, sky, sun) and the text is

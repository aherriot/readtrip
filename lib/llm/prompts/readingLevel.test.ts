...
describe("isReadingLevel", () => {
  it("accepts 1..7 and rejects everything else", () => {
    for (const n of [1, 2, 3, 4, 5, 6, 7]) expect(isReadingLevel(n)).toBe(true);
    for (const n of [0, 8, -1, 2.5, NaN]) expect(isReadingLevel(n)).toBe(false);
  });
});
...
describe("clampReadingLevel", () => {
  it("clamps out-of-range values into 1..7", () => {
    ...
    expect(clampReadingLevel(9)).toBe(7);
  });
...
describe("readingLevelGuidance", () => {
  it("produces distinct, non-empty guidance per level", () => {
    ...
    for (let level = 1 as ReadingLevel; level <= 7; level++) {
